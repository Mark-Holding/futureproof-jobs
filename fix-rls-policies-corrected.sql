-- Fix RLS Policies (Corrected) - Run this in Supabase SQL Editor
-- This will add the missing created_by column and restore proper, secure RLS policies

-- 1. First, add the missing created_by column to companies table
ALTER TABLE companies ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES auth.users(id);

-- 2. Update existing companies to have a created_by value
-- We'll set it to the user who is currently a member of the company
UPDATE companies 
SET created_by = (
  SELECT user_id 
...
-- 3. Make created_by NOT NULL after setting values
ALTER TABLE companies ALTER COLUMN created_by SET NOT NULL;

-- 4. Drop all existing overly permissive policies
...
-- 5. Create proper RLS policies for companies table
...
-- 6. Create proper RLS policies for company_members table
...
-- 7. Create proper RLS policies for job_posts table
...
-- 9. Verify the companies table now has created_by
SELECT 
  id,
  name,
  created_by,
